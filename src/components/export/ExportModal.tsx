...
export function ExportModal({ isOpen, onClose }: Props) {
  ...
  const [targetPlatform, setTargetPlatform] = useState<WhatsAppTarget>("web");
  ...
  const handleDragStart = (e: React.DragEvent) => {
    if (!webpResult) return;
    try {
      e.dataTransfer.setData("DownloadURL", `image/webp:sticker.webp:${webpResult.dataUrl}`);
    } catch {
      // Browser unsupported DownloadURL
    }
    e.dataTransfer.setData("text/uri-list", webpResult.dataUrl);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/60 backdrop-blur-sm animate-in fade-in-50 duration-200">
      ...
      >
        ...
        {/* PRIMARY ACTION: WhatsApp Direct Creation with Step Guide */}
        <div className="space-y-3 p-4 rounded-2xl bg-gradient-to-br from-emerald-500/10 via-emerald-500/5 to-transparent border border-emerald-500/30">
          {/* Target Switcher (Web vs App) */}
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-emerald-900 dark:text-emerald-300 flex items-center gap-1.5">
              <MessageCircle className="w-4 h-4 text-emerald-600" />
              WhatsApp Direct
            </span>
            <div className="flex items-center gap-1 bg-stone-100 dark:bg-[#21262d] p-0.5 rounded-lg text-[10px]">
              <button
                type="button"
                onClick={() => setTargetPlatform("web")}
                className={`px-2 py-0.5 rounded-md font-medium transition-all ${
                  targetPlatform === "web"
                    ? "bg-white dark:bg-[#30363d] text-emerald-700 dark:text-emerald-400 shadow-xs font-bold"
          ...
          <button
            type="button"
            onClick={handleCreateInWhatsApp}
            disabled={isGenerating || !webpResult}
            className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-md shadow-emerald-600/25 active:scale-[0.99] transition-all cursor-pointer group"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <span className="block font-bold">{t("export.createInWhatsApp")}</span>
                <span className="text-[10px] text-white/85 font-normal">
                  {t("export.createInWhatsAppDesc")}
                </span>
              </div>
            </div>
            <span className="text-xs font-mono font-bold px-2.5 py-1 rounded-full bg-white/20">
              {t("export.createInWhatsAppBadge")}
            </span>
          </button>

          {/* Visual Step-by-Step Guide */}
          <div className="p-3 rounded-xl bg-white/80 dark:bg-[#161b22] border border-emerald-500/20 text-xs space-y-2.5 shadow-xs">
            <div className="flex items-center gap-1.5 font-bold text-emerald-800 dark:text-emerald-300 text-[11px]">
              <Sparkles className="w-3.5 h-3.5 text-emerald-600" />
              <span>{t("export.stepGuideTitle")}</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[10px] leading-snug">
              <div className="p-2 rounded-lg bg-stone-50 dark:bg-[#0d1117] border border-stone-200/60 dark:border-[#30363d]">
                <strong className="block text-emerald-600 dark:text-emerald-400 font-bold mb-0.5">
                  {t("export.step1Title")}
                </strong>
                <span className="text-stone-600 dark:text-stone-300">{t("export.step1Desc")}</span>
              </div>
              <div className="p-2 rounded-lg bg-stone-50 dark:bg-[#0d1117] border border-stone-200/60 dark:border-[#30363d]">
                <strong className="block text-emerald-600 dark:text-emerald-400 font-bold mb-0.5">
                  {t("export.step2Title")}
                </strong>
                <span className="text-stone-600 dark:text-stone-300">{t("export.step2Desc")}</span>
              </div>
              <div className="p-2 rounded-lg bg-stone-50 dark:bg-[#0d1117] border border-stone-200/60 dark:border-[#30363d]">
                <strong className="block text-emerald-600 dark:text-emerald-400 font-bold mb-0.5">
                  {t("export.step3Title")}
        ...
        <div className="space-y-2.5">
          {/* Mobile Action: Add to WhatsApp via Sticker Maker (.wastickers) */}
          <button
            type="button"
          ...
          >
            ...
            <span className="text-xs font-mono font-bold px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">
              .wastickers
            </span>
          </button>

          {/* Download Official WebP */}
          <button
            type="button"
          ...
          >
            ...
            <span className="text-[11px] font-mono font-bold text-stone-600 dark:text-stone-400">
              WebP 512
            </span>
          </button>

          {/* Action: Copy Image & Download PNG HD */}
